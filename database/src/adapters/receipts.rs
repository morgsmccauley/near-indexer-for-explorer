        ...
        .filter_map(|(index, r)| {
            ...
            let receipt_or_data_id = match r.receipt {
                near_lake_framework::near_indexer_primitives::views::ReceiptEnumView::Action { .. } => {
                    crate::cache::ReceiptOrDataId::ReceiptId(r.receipt_id)
                }
                near_lake_framework::near_indexer_primitives::views::ReceiptEnumView::Data { data_id, .. } => {
                    crate::cache::ReceiptOrDataId::DataId(data_id)
                }
            };
        ...
        } = &receipt.receipt
        {
            if !output_data_receivers.is_empty() {
                if let Some(transaction_hash) = tx_hashes_for_receipts.get(
                    &crate::cache::ReceiptOrDataId::ReceiptId(receipt.receipt_id),
                ) {
                    for data_receiver in output_data_receivers {
                        receipts_cache_lock.cache_set(
                            crate::cache::ReceiptOrDataId::DataId(data_receiver.data_id),
...
async fn find_tx_hashes_for_receipts(
    ...
    chunk_hash: &near_lake_framework::near_indexer_primitives::CryptoHash,
    receipts_cache: crate::cache::ReceiptsCache,
) -> anyhow::Result<HashMap<crate::cache::ReceiptOrDataId, crate::cache::ParentTransactionHashString>>
{
    let mut tx_hashes_for_receipts: HashMap<
        crate::cache::ReceiptOrDataId,
        crate::cache::ParentTransactionHashString,
    > = HashMap::new();
    ...
    tx_hashes_for_receipts.extend(receipts.iter().filter_map(|receipt| {
        match receipt.receipt {
            near_lake_framework::near_indexer_primitives::views::ReceiptEnumView::Action {
                ...
                .cache_get(&crate::cache::ReceiptOrDataId::ReceiptId(
                    receipt.receipt_id,
                ))
            ...
            near_lake_framework::near_indexer_primitives::views::ReceiptEnumView::Data {
                ...
                receipts_cache_lock
                    .cache_remove(&crate::cache::ReceiptOrDataId::DataId(data_id))
                    .map(|parent_transaction_hash| {
                        (
                            crate::cache::ReceiptOrDataId::DataId(data_id),
    ...
    receipts.retain(|r| match r.receipt {
        near_lake_framework::near_indexer_primitives::views::ReceiptEnumView::Data {
            data_id,
            ..
        } => !tx_hashes_for_receipts.contains_key(&crate::cache::ReceiptOrDataId::DataId(data_id)),
        near_lake_framework::near_indexer_primitives::views::ReceiptEnumView::Action { .. } => {
            !tx_hashes_for_receipts
                .contains_key(&crate::cache::ReceiptOrDataId::ReceiptId(r.receipt_id))
        }
    });
    ...
    loop {
                        ...
                        break res
                            ...
                            .map(
                                |(receipt_id_string, transaction_hash_string): (String, String)| {
                                    (
                                        crate::cache::ReceiptOrDataId::DataId(
            ...
            let tx_hashes_for_receipts_via_data_output: Vec<(
                ...
                .filter_map(|r| {
                    ...
                    near_lake_framework::near_indexer_primitives::views::ReceiptEnumView::Data {
                        ...
                        .get(&crate::cache::ReceiptOrDataId::DataId(data_id))
                        .map(|tx_hash| {
                            (
                                crate::cache::ReceiptOrDataId::ReceiptId(r.receipt_id),
                                tx_hash.to_string(),
        ...
        let tx_hashes_for_receipts_via_outcomes: Vec<(String, crate::cache::ParentTransactionHashString)> =
            crate::await_retry_or_panic!(
                schema::execution_outcome_receipts::table
                    ...
                    .load_async::<(String, crate::cache::ParentTransactionHashString)>(pool),
                10,
        ...
        tx_hashes_for_receipts.extend(tx_hashes_for_receipts_via_outcomes.into_iter().map(
            |(receipt_id_string, transaction_hash_string)| {
                (
                    crate::cache::ReceiptOrDataId::ReceiptId(
                        near_lake_framework::near_indexer_primitives::CryptoHash::from_str(
                            &receipt_id_string,
                        )
                        .expect("Failed to convert String to CryptoHash"),
